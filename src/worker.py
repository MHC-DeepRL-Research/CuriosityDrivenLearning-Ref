...
def run(args, server):
    # step1: creates an RL environment
    ...
    # step4: creates tensorflow variable initializer object
    #        use_tf12_api is a flag that checks tf version (because syntaxes are different)
    #        variable names that start with "local" are not saved in checkpoints.
    if use_tf12_api:
        variables_to_save = [v for v in tf.global_variables() if not v.name.startswith("local")]
        init_op = tf.variables_initializer(variables_to_save)  # used in line 99 (updates periodically)
        init_all_op = tf.global_variables_initializer()        # used in line 71 (only updates at the start)
    else:
        variables_to_save = [v for v in tf.all_variables() if not v.name.startswith("local")]
        init_op = tf.initialize_variables(variables_to_save)
        init_all_op = tf.initialize_all_variables()
        
    # step5: creates an efficient parameter saver object
    saver = FastSaver(variables_to_save)   # used in line 97
    
    # step6: load pretrained parameters (if any)
    ...
    # step8: creates an initialization function (used in line 100)
    def init_fn(ses):
    ...
    # step9: training configuration set up (used in line 114)
    config = tf.ConfigProto(device_filters=["/job:ps", "/job:worker/task:{}/cpu:0".format(args.task)])
    
    # step10: defines a log directory (used in line 96) and a summary writer (used in line 101)
    ...
    with sv.managed_session(server.target, config=config) as sess, sess.as_default():
        ...
        # step12-3: keeps running more trainingn steps until stop condition satisfies
        while not sv.should_stop() and (not num_global_steps or global_step < num_global_steps):
            trainer.process(sess)
            global_step = sess.run(trainer.global_step)

    # step13: ask for all the services to stop.
    sv.stop()
...
def main(_):
    ...
    # set up signals to stop the process
    ...
    # create train server for workers
    ...
    # create train server for parameter server
    else:
